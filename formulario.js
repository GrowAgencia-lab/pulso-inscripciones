...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.getElementById('tipo-part').addEventListener('change',calcularCosto);
...
function buscarCI(){
  ...
  if(p){
    ...
    res.innerHTML=`<div class="result-nf">
      ...
      <div style="font-size:.82rem;color:#555;margin-bottom:.5rem">Este participante no está en el sistema.</div>
...
function buscarNombre(){
  ...
  const matches=Object.entries(DB).filter(([ci,p])=>{
    ...
    return p.nombre.toLowerCase().includes(q) ||
           (p.sinCI && ci.toLowerCase().replace('sinci_','').replace(/_/g,' ').includes(q));
  });
  if(!matches.length){
  ...
  resEl.innerHTML=matches.slice(0,8).map(([ci,p])=>{
    const ya=listaParticipantes.some(x=>x.ci===ci);
...
function renderLista(){
  document.getElementById('lista-count').textContent=listaParticipantes.length;
...
function calcularCosto(){
  ...
  const cant=listaParticipantes.length||1;
  const hoy=new Date();
  const c1=new Date(cfg.formulario.fecha_1er_llamado_cierre);
