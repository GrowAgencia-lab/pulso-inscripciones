...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  cargarDesdeStorage();
  // La Lista de Espera no respeta los cierres del formulario principal: siempre
  // queda disponible, así sirve para anotar interesados aunque esté cerrado.
  verificarLlamado();
  inyectarImagenes();
});
...
function verificarLlamado(){
  if(!cfg)return;
  const hoy=new Date();
  const c1=new Date(cfg.formulario.fecha_1er_llamado_cierre+'T23:59:59');
  const es1=hoy<=c1;
...
function poblarSelects(){
  ...
  document.getElementById('categoria').addEventListener('change',actualizarDisponibilidadTipo);
  actualizarDisponibilidadTipo();
}

// ── CIERRES PARCIALES DE INSCRIPCIÓN (por modalidad/categoría/tipo) ──
const TIPOS_PART=[
  {value:'solista',    label:'Solista'},
  {value:'duo',        label:'Dúo (2 participantes)'},
  {value:'trio',       label:'Trío (3 participantes)'},
  {value:'cuarteto',   label:'Cuarteto (4 participantes)'},
  {value:'grupo',      label:'Grupo (5 a 10 participantes)'},
...
function getTiposCerrados(modalidad,categoria){
  // Lista de Espera: nunca cierra nada — siempre se puede elegir cualquier
  // modalidad/categoría/tipo, esté o no cerrado en el formulario principal.
  return [];
}

function actualizarDisponibilidadTipo(){
  const sel=document.getElementById('tipo-part');
  const aviso=document.getElementById('aviso-cierre');
  if(!sel||!aviso) return;
  const modalidad=document.getElementById('modalidad').value;
  const categoria=document.getElementById('categoria').value;
  const valorActual=sel.value;
  const cerrados=getTiposCerrados(modalidad,categoria);

  sel.innerHTML='<option value="">— Seleccioná —</option>';
  TIPOS_PART.forEach(t=>{
    if(cerrados.includes(t.value)) return; // no se agrega = no se puede elegir
    const o=document.createElement('option');
    o.value=t.value; o.textContent=t.label;
    sel.appendChild(o);
  });

  if(valorActual&&cerrados.includes(valorActual)){
    sel.value='';
    const campoCant=document.getElementById('campo-cantidad');
    if(campoCant) campoCant.style.display='none';
    const inputCant=document.getElementById('cant-participantes');
  ...
  if(!modalidad||!categoria||!cerrados.length){ aviso.style.display='none'; return; }
...
async function subirArchivoDrive(file, inscId, tipo, obra, modalidad, categoria, academia){
  ...
  for(let i=0; i<totalChunks; i++){
    ...
    const payload = {
      accion:      'subirChunk',
      hoja:        'Inscripción Lista de Espera',
      id:          inscId,
      tipo,
      chunk:       i,
      totalChunks,
...
async function enviar(e){
  ...
  try{
    ...
    if(json && json.ok===false){
      alert('Hubo un problema al guardar tu inscripción: '+(json.error||'error desconocido')+'. Por favor, intentá de nuevo. Si el problema persiste, escribinos por WhatsApp al +595986250000.');
      btn.disabled=false;
      return;
    }
    if(json && json.id) datos.id=json.id;
  }catch(err){
    alert('No pudimos conectar con el servidor para guardar tu inscripción. Revisá tu conexión a internet e intentá de nuevo. Si el problema persiste, escribinos por WhatsApp al +595986250000.');
    btn.disabled=false;
    return;
  }
