...
async function enviar(e){
  ...
  // Leer MP3 si fue seleccionado
  const inputMp3=document.getElementById('upload-musica');
  if(inputMp3&&inputMp3.files.length&&datos.tipo_musica==='mp3'){
    try{
      txt.textContent='Subiendo música...';
      datos.musica_b64=await fileToBase64(inputMp3.files[0]);
      datos.musica_nombre=inputMp3.files[0].name;
      datos.musica_size=inputMp3.files[0].size;
    }catch(err){console.warn('Error leyendo MP3:',err);}
  }

  // Leer comprobante si fue seleccionado (opcional)
  ...
  if(inputComp&&inputComp.files.length){
    try{
      ...
      datos.comp_b64=await fileToBase64(inputComp.files[0]);
      datos.comp_nombre=inputComp.files[0].name;
      datos.comp_tipo=inputComp.files[0].type||'image/jpeg';
    }catch(err){console.warn('Error leyendo comprobante:',err);}
  }
